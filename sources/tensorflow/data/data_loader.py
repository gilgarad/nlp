...
class DataLoader:
    ...
    def __init__(self, data_path, test_path, hparams, output_path=None):
        ...
        self.dataset = DataSet(data_path=data_path, hparams=hparams)

        # self.label_list = self.dataset.label_dict.keys()

        if data_path != test_path:
            self.test_dataset = DataSet(data_path=test_path, hparams=hparams)
        else:
            # test mode
            ...
            print('test mode or train without cross validation')
